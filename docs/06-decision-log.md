...
- **NEXT (in progress):** Stage 5 **step A done** (D32) and **step C done** (D33 EC pack + D35 AU pack & Look non-code) — Method Library §5 now carries both packs + both source classes. **Remaining: step B** — task-level In/Does/Out/Gates procedures for 5.0–5.7 (mirroring `02-interpretation-procedures.md`), 5.2 (bearing + safety format) & 5.5 (settlement) deeper. Optional: author a parallel AU φg worked calibration (AS 5100.3 ships none). Then repeat the three-step treatment for **axial pile** (Stage 6) — JRC 2013 Ch 8/Annex A.8 has worked pile examples to reuse.
